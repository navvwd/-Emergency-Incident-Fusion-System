...
exports.FUSION_THRESHOLD = exports.FUSION_WEIGHTS = void 0;
exports.cosineSimilarity = cosineSimilarity;
exports.haversineDistance = haversineDistance;
exports.geoScore = geoScore;
exports.normalizeLocation = normalizeLocation;
exports.locationStringScore = locationStringScore;
exports.temporalScore = temporalScore;
exports.categoryScore = categoryScore;
exports.extractSignificantTokens = extractSignificantTokens;
exports.entityScore = entityScore;
exports.computeFusionScore = computeFusionScore;
exports.recalculateSeverity = recalculateSeverity;
exports.applyConfidenceEscalation = applyConfidenceEscalation;
exports.buildCandidateForScoring = buildCandidateForScoring;
// ─── Constants (configurable via env) ──────────────────────
exports.FUSION_WEIGHTS = {
...
exports.FUSION_THRESHOLD = parseFloat(process.env.FUSION_THRESHOLD || '0.72');
...
function geoScore(lat1, lng1, lat2, lng2) {
...
function locationStringScore(loc1, loc2) {
...
function temporalScore(time1, time2) {
...
// ─── 4. Categorical Match ───────────────────
function categoryScore(type1, type2) {
    if (type1 === type2)
        return 1.0;
...
function entityScore(extracted1, extracted2) {
    let score = 0;
    let factors = 0;
...
function computeFusionScore(newReport, existingIncident) {
    // 1. Semantic similarity
    const semantic = cosineSimilarity(newReport.embedding, existingIncident.embedding);
    // 2. Geospatial proximity
    let geospatial;
    if (newReport.extracted.latitude && newReport.extracted.longitude &&
        existingIncident.latitude && existingIncident.longitude) {
        geospatial = geoScore(newReport.extracted.latitude, newReport.extracted.longitude, existingIncident.latitude, existingIncident.longitude);
    }
    else {
        geospatial = locationStringScore(newReport.extracted.location, existingIncident.extracted.location);
    }
    // 3. Temporal proximity
    const temporal = temporalScore(newReport.created_at, existingIncident.created_at);
    // 4. Categorical match
    const categorical = categoryScore(newReport.extracted.incident_type, existingIncident.extracted.incident_type);
    // 5. Entity overlap
    const entity = entityScore(newReport.extracted, existingIncident.extracted);
    // Composite score
    const score = exports.FUSION_WEIGHTS.semantic * semantic +
        exports.FUSION_WEIGHTS.geospatial * geospatial +
        exports.FUSION_WEIGHTS.temporal * temporal +
        exports.FUSION_WEIGHTS.categorical * categorical +
        exports.FUSION_WEIGHTS.entity * entity;
...
function recalculateSeverity(existingSeverity, existingReportCount, newSeverity) {
...
function applyConfidenceEscalation(currentSeverity, _reportCount, allSeverities) {
...
function buildCandidateForScoring(row, candidateEmbedding) {
    return {
        extracted: {
